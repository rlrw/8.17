                ...
                <div style="width: 40%;float: left;">
                    <div class="panel panel-default  no-boder bg-color-blue">
                        <div class="panel-body" style=" font-size: 18px">
                           <span class="col-sm-6" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #4CB1CF;">直接：</text><?php echo ($ztpay); ?></span>
                            <span class="col-sm-6" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #4CB1CF;">间接：</text><?php echo ($jtpay); ?></span>
                            <span class="col-sm-6" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #4CB1CF;">级差：</text><?php echo ($jcpay); ?></span>
                            <span class="col-sm-6" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #4CB1CF;">分红：</text><?php echo ($fhpay); ?></span>
                            <span class="col-md-12 col-sm-12"><text style="color: #4CB1CF;">服务中心：</text><?php echo ($fwpay); ?></span>
                        </div>
                        <div class="panel-footer back-footer-blue text-center">
                            总收益（未扣税）= 合计：<?php echo ($zpay); ?>
                        </div>
                    </div>
                </div>
                <div style="width: 5%;float: left;text-align: center;"><span style="padding: 80px 0; display: block; font-weight: bold; font-size: 28px;">-</span></div>
                <div style="width: 30%;float: left;">
                    <div class="panel panel-default no-boder bg-color-red">
                        <div class="panel-body" style="font-size: 20px; padding: 40px 15px 19px 15px;">
                            <span class="col-sm-12" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #F0433D;">扣税：</text><?php echo ($tax); ?></span>
                            <span class="col-sm-12" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee"><text style="color: #F0433D;">扣重消：</text><?php echo ($nopay); ?></span>
                        </div>
                        <div class="panel-footer back-footer-red text-center">
                            扣税 / 重消 = 合计：<?php echo ($kcpay); ?>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $(function(){
                        ...
                        if(data){
                            if(a==1){
                                ...
                                $.each(data,function(k,v){
                                    tr+="<tr><td>"+v.new_time+"</td><td>"+v.money+"</td><td>"+v.tax+"</td><td>"+ v.no_points+"</td><td>"+ v.pay_points+"</td></tr>";
                                });
            ...
            $.ajax({
                url:"<?php echo U('List/get_month');?>",
                type:"post",
                success:function(e){
                    console.log(e);
                    var arr=[];
                    for(var i in e){
                        arr.push(parseFloat(e[i])/10000);
                    }
                    ...
                    $(document).ready(function(){
                        var date=new Date();
                        var math=date.getMonth()+1;
                        $(".panel-heading span").text(math);
                        $(".highcharts-tracker rect").click(function () {
                             math=$(this).index()+1;
                            $(".panel-heading span").text(math);
                            test(math);
                        });
